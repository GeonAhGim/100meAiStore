...
class DevDashboardCollector:
    ...
    def _development_progress(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"available": False, "total": None, "completed": None, "in_progress": None,
                    ...
                    "requirements": self._requirements_progress()}
    ...
    def _requirements_progress(self) -> dict[str, Any]:
        """Summarize the validated atomic ledger; invalid evidence never yields 100%."""
        manifest = self.project_root / "docs" / "implementation" / "requirements-traceability.json"
        try:
            from scripts.validate_traceability import validate_manifest
            errors = validate_manifest(manifest, self.project_root)
            value = json.loads(manifest.read_text(encoding="utf-8"))
            items = value["requirements"]
            if errors or not isinstance(items, list):
                raise ValueError("invalid traceability manifest")
        except (ImportError, OSError, UnicodeError, ValueError, TypeError, KeyError, json.JSONDecodeError):
            return {"available": False, "valid": False, "total": None, "completed": None,
                    "partial": None, "planned": None, "approval_gated": None, "percent": None,
                    "complete_claim_allowed": False}
        counts = {status: sum(item.get("status") == status for item in items)
                  for status in ("completed", "partial", "planned", "approval_gated")}
        total = len(items)
        completed = counts["completed"]
        return {"available": True, "valid": True, "total": total, "completed": completed,
                **{key: counts[key] for key in ("partial", "planned", "approval_gated")},
                "percent": round(completed * 100 / total) if total else None,
                "complete_claim_allowed": total == 43 and completed == total}
