...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 - stdlib handler API
        ...
        try:
            ...
            # Authentication was already established above. Collection
            # authorization is explicit, while detail authorization is made
            # indistinguishable from an absent/cross-tenant resource.
            if approval_match:
                self._send_json(404, {"error": "not_found"})
            else:
                self._send_json(403, {"error": "forbidden"})
        except (NotFoundError, TenantBoundaryError):
            self._send_json(404, {"error": "not_found"})
