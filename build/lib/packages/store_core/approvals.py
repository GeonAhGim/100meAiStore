...
def approval_inbox(service: Any, context: Any) -> dict[str, Any]:
    ...
    readable = {capability.value for capability in APPROVAL_CAPABILITY.values()}
    if Capability.READ_AUDIT.value not in permissions and not readable.intersection(permissions):
