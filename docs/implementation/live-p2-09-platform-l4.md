...
## Authenticated browser and confirmation boundary

The dashboard no longer accepts tenant/user identity in query parameters or
browser storage. `/api/dashboard` resolves an opaque `store_session` cookie
against a server-side SHA-256 digest and persisted tenant/user/membership
version. Anonymous, expired, malformed and revoked/stale sessions fail with no
tenant lookup from caller-controlled identity. The trusted identity callback
contract emits `HttpOnly; Secure; SameSite=Strict` cookies; this repository does
not claim to perform email/MFA verification itself.

Approval confirmation nonces are generated by the server, stored only as
digests, bound to session/tenant/approval/command, expire within five minutes
and are consumed once with an atomic compare-and-set. The browser endpoints
require same-origin JSON. Wrong-session, cross-approval, arbitrary, expired and
replayed nonce attempts cannot decide an approval. The legacy mobile decision
entry point now always fails closed; internal fixture workflows use the core
domain transition and browser callers use the authenticated transition.

SQLite restart preserves sessions and nonce consumption. Seven focused auth/
HTTP tests cover raw-ID denial, secure-cookie flags, three-user membership
revocation, restart, one-use confirmation and browser-shell storage. External
identity/MFA callback, deploy-time TLS and a complete PWA remain A-002/G3 work.
