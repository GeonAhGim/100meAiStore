...
class SQLiteRepositoryTest(unittest.TestCase):
    ...
    def test_completed_is_terminal_and_v1_database_migrates_on_reopen(self):
        ...
        self.assertEqual([version for version, _ in MIGRATIONS], versions)
    ...
    def test_migration_ddl_and_version_marker_roll_back_together(self):
        ...
        broken = MIGRATIONS + ((broken_version, "CREATE TABLE must_not_survive(id TEXT); THIS IS INVALID;"),)
        ...
        try:
            ...
            self.assertIsNone(connection.execute("SELECT version FROM schema_migrations WHERE version=?", (broken_version,)).fetchone())
        finally:
            connection.close()
